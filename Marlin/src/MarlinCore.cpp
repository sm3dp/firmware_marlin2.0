...
#if ENABLED(LOTMAXX_LASER_PRINTMODE)
...
void setup() {
  ...
  // LASER 
  // init laser min pin and detect laser
  // must be before EEPROM load 
  // (because of the settings of recovery and runout saved in EEPROM  Don't be expected)

  #if ENABLED(LOTMAXX_LASER_PRINTMODE)
    #if PIN_EXISTS(LASER_MIN)
      SET_INPUT_PULLUP(LASER_MIN_PIN);
    #endif 
    #if PIN_EXISTS(LASER_DET)
      SET_INPUT_PULLUP(LASER_DET_PIN);
      safe_delay(50);
      if(READ(LASER_DET_PIN) == LASER_ACTIVE_STATE){
        printmode.set_printmode(LASER);
